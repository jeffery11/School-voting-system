...
app.get('/api/admin/dashboard', requireAdmin, (req, res) => {
  db.serialize(() => {
    db.get('SELECT COUNT(*) as total_voters FROM voters', (err, voters) => {
      db.get('SELECT COUNT(*) as total_candidates FROM candidates', (err, candidates) => {
        db.get('SELECT COUNT(*) as votes_cast FROM votes', (err, votes) => {
          db.get('SELECT COUNT(*) as voted_count FROM voters WHERE has_voted = 1', (err, voted) => {
            res.json({
              total_voters: voters.total_voters,
              total_candidates: candidates.total_candidates,
              votes_cast: votes.votes_cast,
              voted_count: voted.voted_count,
              participation_rate: voters.total_voters > 0 ? ((voted.voted_count / voters.total_voters) * 100).toFixed(1) : 0
            });
          });
        });
      });
...
app.post('/api/admin/candidates', requireAdmin, (req, res) => {
  const { name, position, grade, description, campaign_slogan } = req.body;
  if (!name || !position || !grade) {
    return res.status(400).json({ error: 'Name, position, and grade are required' });
  }
  db.run(
    'INSERT INTO candidates (name, position, grade, description, campaign_slogan) VALUES (?, ?, ?, ?, ?)',
    [name, position, grade, description, campaign_slogan],
    function(err) {
      if (err) {
...
app.post('/api/admin/voters', requireAdmin, (req, res) => {
  const { name, student_id, grade_level } = req.body;
  if (!name || !student_id || !grade_level) {
    return res.status(400).json({ error: 'Name, student ID, and grade level are required' });
  }
  const defaultPassword = bcrypt.hashSync('password123', 10);
  db.run(
    'INSERT INTO voters (name, student_id, password_hash, grade_level) VALUES (?, ?, ?, ?)',
    [name, student_id, defaultPassword, grade_level],
    function(err) {
      if (err) {
        return res.status(500).json({ error: 'Failed to add voter. Student ID might already exist.' });
      }
      res.json({
        message: 'Voter added successfully',
        voter_id: this.lastID
      });
    }
  );
});
...
app.post('/api/admin/reset-voting', requireAdmin, (req, res) => {
  db.serialize(() => {
    db.run('DELETE FROM votes', (err) => {
      if (err) {
        return res.status(500).json({ error: 'Failed to reset votes' });
      }
      db.run('UPDATE voters SET has_voted = FALSE', (err) => {
        if (err) {
          return res.status(500).json({ error: 'Failed to reset voter status' });
        }
        res.json({ message: 'Voting reset successfully' });
      });
    });
  });
});

// Get all votes (for admin viewing)
